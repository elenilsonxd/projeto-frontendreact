  ...
  return (
    <>
      ...
      <MainContainer>
        <div
          className={
            context.showModal ? "menu-overlay-open" : "menu-overlay-close"
          }
        ></div>
        ...
        <SectionButtons>
          ...
          {context.pageNumber !== 1 && (
            <button onClick={() => handlePageTurn(0)}>{"<<"}</button>
          )}
          {context.pageNumber - 2 > 0 && (
            <button onClick={() => handlePageTurn(-2)}>
              {context.pageNumber - 2}
            </button>
          )}
          {context.pageNumber - 1 > 0 && (
            <button onClick={() => handlePageTurn(-1)}>
              {context.pageNumber - 1}
            </button>
          )}
          <button>{context.pageNumber}</button>
          {context.pageNumber + 1 < context.totalPages && (
            <button onClick={() => handlePageTurn(1)}>
              {context.pageNumber + 1}
            </button>
          )}
          {context.pageNumber + 2 < context.totalPages && (
            <button onClick={() => handlePageTurn(2)}>
              {context.pageNumber + 2}
            </button>
          )}
